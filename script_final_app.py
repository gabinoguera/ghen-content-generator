...
def gemini_summarize_and_analyze(text):
    try:
        ...
        st.error(f"Error al generar el análisis con Gemini 1.5 Flash: {str(e)}")
        return "Error al generar análisis con Gemini 1.5 Flash"

def process_scraped_articles(df):
    if df.empty:
        st.warning("El DataFrame está vacío, no hay datos para procesar.")
        return df

    for index, row in df.iterrows():
        article_text = row['scraped_text']
        if not article_text.strip():
            st.warning(f"Artículo vacío para la URL: {row['link']}, omitiendo análisis.")
            df.at[index, 'SEO Analysis'] = "No se pudo scrapear el texto del artículo"
            continue

        st.write(f"Procesando artículo {index + 1}/{len(df)}: {row['link']}")
    ...
    st.success(f"Archivo CSV generado: {output_csv}")
    return df

def generar_guia_escritor_con_gemini(df):
    if 'SEO Analysis' not in df.columns:
        st.warning("La columna 'SEO Analysis' no está presente en el DataFrame.")
        return
    ...
    try:
        ...
        st.success("Guía para el escritor generada con éxito.")
...
def generar_contenido_seccion(seccion, guia_para_escritor):
    ...
    3. Reducir el formato de listas cuando no sean necesarias, prefiero párrafos.
    ...
    try:
        ...
        st.success(f"Contenido generado con éxito para la sección: {seccion[:50]}...")
        return contenido_seccion
    except Exception as e:
        st.error(f"Error al generar contenido para la sección {seccion[:50]}...: {str(e)}")
...
# Inicializa la variable `contenido_editado` en el inicio del script
contenido_editado = ""

# Combinar las secciones mejoradas en un solo contenido
contenido_completo = "\n\n".join(contenido_editado)
...
# Sección para generar guía y esquema
if st.button("Generar Guía y Esquema"):
    ...
    # Permitir editar el esquema
    esquema_editado = st.text_area("Edita el esquema si es necesario:", value=esquema_articulo, height=300)

    # Generar contenido tras editar el esquema
    if st.button("Generar Contenido"):
        contenido_procesado = procesar_esquema(esquema_editado, guia_para_escritor)
        contenido_completo = "\n\n".join(contenido_procesado)
        
        # Permitir revisar y editar el contenido antes de guardarlo
        ...
        if st.checkbox("Publicar en WordPress"):
            ...
            if st.button("Publicar"):
                publicar_en_wordpress(titulo, contenido_editado)  # Solo se llama aquí
